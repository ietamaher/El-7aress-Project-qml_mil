#!/usr/bin/env python3
"""
Servo Elevation Tester - Elevation Servo Driver
Tests the elevation servo via Modbus RTU
...
def load_config():
    ...
    try:
        with open(config_path, 'r') as f:
            config = json.load(f)
            return config['servo']['elevation']
    except FileNotFoundError:
        print(f"ERROR: Configuration file not found: {config_path}")
        sys.exit(1)
    except KeyError:
        print("ERROR: 'servo.elevation' configuration not found in devices.json")
        sys.exit(1)

# --- Servo Modbus Addresses ---
ADDR_POSITION = 0x0064
ADDR_VELOCITY = 0x0065
ADDR_STATUS = 0x0010
ADDR_ALARM = 0x0011
ADDR_MOTOR_TEMP = 0x0020
ADDR_DRIVER_TEMP = 0x0021
...
def main():
    ...
    print("=" * 70)
    print("  SERVO ELEVATION TESTER - Elevation Axis Driver")
    print("=" * 70)
